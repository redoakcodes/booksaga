  ...
  function ensureDir(
    ...
    for (let i = 0; i < upTo; i++) {
      ...
      let dir = current.find(
        (n): n is TreeDir => n.kind === "dir" && n.name === parts[i],
      );
      if (!dir) {
...
const FolderIcon = () => (
  <svg
    class="wiki-file-icon wiki-folder-icon"
    viewBox="0 0 14 14"
    xmlns="http://www.w3.org/2000/svg"
    aria-hidden="true"
  >
    <path
      d="M1 4.5 Q1 3.5 2 3.5 L5 3.5 L6.2 4.8 L12 4.8 Q13 4.8 13 5.8 L13 11 Q13 12 12 12 L2 12 Q1 12 1 11 Z"
      fill="none"
      stroke="currentColor"
      stroke-width="1.2"
    />
        ...
        <li
          ...
          onContextMenu={(e) =>
            props.onContextMenu(
              e,
              fileParentDir(),
              (props.node as TreeFile).path,
              "file",
            )
          }
        >
          <span class="toc-expand-cell" />
          {(props.node as TreeFile).name.endsWith(".mmd") ? (
            <DiagramIcon />
          ) : (
            <WikiIcon />
          )}
          <span class="toc-label">
            {displayName((props.node as TreeFile).name)}
          </span>
        </li>
      </Show>
    </>
...
const WikiTree: Component<Props> = (props) => {
  const tree = () =>
    buildTree([...props.files, ...(props.diagramFiles ?? [])], props.dirs);
